...
Rules:
- AREA NAME must be plain English (e.g. "Disinfectant residual monitoring") — no citation codes in the area name
...
def parse_suggestions(raw: str) -> list:
    ...
    for line in raw.strip().split("\n"):
        ...
        if not line or "|" not in line:
            continue
        # Skip markdown table headers and separator lines
        if re.match(r'^[-|\s]+$', line):
            continue
        ...
        if len(parts) >= 3:
            ...
            if "HIGH" not in likelihood and "MODERATE" not in likelihood:
                continue
            # Skip if code field doesn't look like a citation
            code = parts[0]
